...
# Lancer Hashcat
echo "[HASHCAT ACTION] hashcat $algorithm $path_hash $final_wordlist_file --status -O --status-timer 1 | tee -a $log_hashcat | grep -E 'Progress|Estimated|Speed'  > $path_parsed_output_hashcat "
